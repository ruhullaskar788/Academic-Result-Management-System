...
export const adminLogin = async (req, res) => {
  ...
  try {
    ...
    const isPasswordCorrect = password.localeCompare(existingAdmin.password);
...
export const addAdmin = async (req, res) => {
  try {
    console.log("Request add admin0");
    const { name, dob, department, username, password, contactNumber, avatar, email, joiningYear } =
      req.body;
      console.log(name);
      ...
      if (existingDepartment) {
        ...
        departmentHelper = "DEFAULT"; // Set a default value or handle it as per your requirement
      }
    ...
    //var username = "abc";//components.join("");
    let hashedPassword;
    const newDob = dob.split("-").reverse().join("-");

    //hashedPassword = await bcrypt.hash(newDob, 10);
    console.log("Request add admin2");
...
export const addStudent = async (req, res) => {
  try {
    ...
    // hashedPassword = await bcrypt.hash(newDob, 10);
    var password = fatherContactNumber;
    var username= fatherContactNumber;
